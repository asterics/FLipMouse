...
def main():
    ...
    while 1:
      ...
      if startwith <= 1:
        print("Part 1 ###############################################")
        proc = subprocess.run(["python3","uf2conv.py","serialflasher1.uf2","-s", current_serial,"-D"], capture_output=True, text=True)
        print(proc.stdout)
      ...
      if startwith <= 2:
        ...
        while 1:
          ...
          proc = subprocess.run(["esptool.py","--port", current_serial,"--baud", "115200", "--before", "no_reset", "--after", "no_reset", "--chip", "esp32", "write_flash", \
      ...
      if startwith <= 3:
        print("Part 3 ###############################################")
        proc = subprocess.run(["python3","uf2conv.py","serialflasher2.uf2","-s", current_serial,"-D"], capture_output=True, text=True)
        print(proc.stdout)
        if proc.returncode != 0:
          print("Error flashing UF2 'serialflasher1.uf2'")
          exit
      ...
      if startwith <= 6:
        print("Part 6 ###############################################")
        proc = subprocess.run(["python3","uf2conv.py",current_device + ".uf2","-s", current_serial,"-D"], capture_output=True, text=True)
        print(proc.stdout)
        if proc.returncode != 0:
          print(proc.stderr)
